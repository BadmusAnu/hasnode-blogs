...
DataStream is an abstraction layer for managing append-only time-series data across multiple "backing indices."  
...
Index Templates: Predefined blueprints used to automatically initialize new indices with consistent settings (shards, replicas) and mappings. See example code below.  
