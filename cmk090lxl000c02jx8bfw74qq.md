...
DataStream is an abstraction layer for managing append-only time-series data across multiple "backing indices." One of the core caveats of datastream is that a timestamp field must always be present, and once defined, that field can’t change. That is the reason why there only usable with time-series data.
